...
def get_flood_data():
    """
    Main scraper entry used by main.py.

    Railway does not always have Tesseract/Chrome installed, so OCR must not
    crash the whole scraper. If OCR fails, continue with direct PMD table parsing.
    """
    ...
    try:
        ...
        print(f"⚠️ OCR scraper failed, continuing with direct PMD parser: {e}")
    ...
    if not result:
        raise RuntimeError(
            "No station readings found from OCR or direct PMD parser."
        )
