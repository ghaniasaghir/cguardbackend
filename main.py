...
def scrape_now(db: Session = Depends(get_db)):
    """
    Runs OCR scraper, saves valid readings,
    predicts flood forecast using trained models.
    """
    ...
    try:
        ...
        if not scraped_rows:
            raise HTTPException(
                status_code=500,
                detail="Scraper returned no data."
            )

        saved_results = []

        # Handle dict OR list scraper response
        if isinstance(scraped_rows, dict):
            rows_to_process = scraped_rows.values()
        else:
            rows_to_process = scraped_rows

        for row in rows_to_process:

            try:
                station = normalize_station_name(
                    str(row.get("station", "")).strip()
                )

                # OCR sometimes returns inflow instead of discharge
                discharge_value = (
                    row.get("discharge")
                    or row.get("inflow")
                    or 0
                ...
                # SKIP BAD OCR VALUES
                if discharge <= 0:
                    print(f"⚠️ Skipping {station} because discharge is 0")
                    continue

                if discharge > 1000000:
                    print(f"⚠️ Skipping {station} because discharge is unrealistic")
                    continue

                rainfall = float(row.get("rainfall_mm", 0) or 0)
                temperature = float(row.get("temperature_c", 25) or 25)
                soil = float(row.get("soil_moisture_mm", 25) or 25)

                reading_time = parse_scraper_datetime(
                    row.get("date")
                )
                ...
                forecast = predict_from_station_reading(
                    db,
                    reading
                )

                safe_forecast = make_json_safe(forecast)
                ...
                print(f"✅ Saved {station} successfully")
        ...
        return {
            "success": True,
            "saved_count": int(len(saved_results)),
            "results": make_json_safe(saved_results)
        }
