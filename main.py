...
def predict_from_station_reading(db: Session, reading: StationReadingDB):
    # IMPORTANT:
    # Do NOT call weather.py/Open-Meteo inside Railway prediction endpoints.
    # Weather values are already saved in station_readings by run_scraper_and_upload.py.
    # Calling live weather here caused Open-Meteo timeouts and DB QueuePool 500 errors.
...
def get_live_station_weather_values(station: str):
    """
    Returns weather values in the exact names required by ML features.
    Priority: weather.py live endpoint -> STATION_WEATHER fallback.
    This prevents prediction endpoints from always showing temperature/soil moisture as 25.
    """
    station = normalize_station_name(station)
    ...
    # Support multiple key styles returned by weather.py/frontend.
    temperature = (
...
def attach_live_weather_to_reading(reading: StationReadingDB):
    """
    Creates an in-memory reading for prediction using trusted DB discharge
    plus live weather values. It does not write anything to the database.
    """
