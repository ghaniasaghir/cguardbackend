...
def build_uc_risk_snapshot_from_readings(
    ...
    if reading:
        ...
        current_discharge = float(STATION_DEFAULT_DISCHARGE.get(station, 5000))
    ...
    # Force Normal for all monitored UCs
    risk_percentage = 12
    risk_level = 'Normal'
    return {
        "id": uc_id,
...
def calculate_chenab_overall_risk(db: Optional[Session] = None):
    ...
    for item in snapshots:
            label = str(item.get("risk_level", "Normal")).strip().title()
            counts[label_to_count.get(label, "NORMAL")] += 1
