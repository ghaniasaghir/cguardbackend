...
# ----- Force all 789 UCs to Normal -----
from sqlalchemy.orm import sessionmaker

Session = sessionmaker(bind=engine)
session = Session()

# Assuming UC_RiskDB is the model for UC risk table
for uc in session.query(UC_RiskDB).all():
    uc.risk_level = "Normal"
    uc.risk_percentage = 12
    session.add(uc)
...
class FloodDataDB(Base):
    ...
    current_discharge = float(reading.discharge) if reading and reading.discharge is not None else STATION_DEFAULT_DISCHARGE.get(station, 5000)
...
# Only trusted production upload sources are treated as real readings.
# Old OCR/direct Railway scraper rows are excluded because they produced wrong/duplicate values.
REAL_READING_SOURCES = [
    "scheduled_scraper",
    "manual_scraper",
    "curl_cffi_scraper",
    "local_scheduled_scraper",
    "manual",
    "api_manual",
]
...
groq_client = Groq(api_key=GROQ_API_KEY, timeout=20.0) if GROQ_API_KEY else None
...
def seed_union_councils_and_shelters():
    ...
    try:
        ...
        if db.query(ShelterDB).count() == 0:
            shelters = [
                ShelterDB(
                    name="Trimmu Flood Relief Camp",
                    location="Sports Complex, Jhang",
                    district="Jhang",
                    capacity=500,
                    occupied=0,
                    status="Available",
...
# ─────────────────────────────────────────────
# TEMP FIX: RESET SHELTERS TO ONLY TRIMMU
# Deploy this file, open /docs, run POST /debug/reset-shelters-to-trimmu once.
# After it works, remove this endpoint for production safety.
# ─────────────────────────────────────────────
@app.post("/debug/reset-shelters-to-trimmu")
def reset_shelters_to_trimmu(db: Session = Depends(get_db)):
    """
    Deletes all existing shelter rows from the connected database and inserts
    only Trimmu Flood Relief Camp. This fixes old shelters that were already
    saved before the seed code was changed.
    """
    deleted_count = db.query(ShelterDB).delete()
    db.commit()
    ...
    return {
        "success": True,
        "message": "Shelters reset successfully. Only Trimmu Flood Relief Camp remains.",
        "deleted_old_shelters": deleted_count,
        "shelter": {
            "id": shelter.id,
            "name": shelter.name,
            ...
            "facilities": json.loads(shelter.facilities or "[]"),
            "uc_id": shelter.uc_id,
        }
    }

@app.get("/debug/current-shelters")
def current_shelters_debug(db: Session = Depends(get_db)):
    """Quick check endpoint to confirm which shelters are in the connected DB."""
    shelters = db.query(ShelterDB).all()
    return {
        "count": len(shelters),
        "shelters": [
            {
                ...
                "occupied": s.occupied,
                "status": s.status,
                "facilities": json.loads(s.facilities or "[]"),
                "uc_id": s.uc_id,
            }
            for s in shelters
        ]
...
DISCHARGE_THRESHOLDS = {
    # ML-partner approved discharge thresholds for C-Guard risk scale.
    # These keep risk classification aligned with predicted discharge values.
    "low": 100000,
    "medium": 150000,
    "high": 200000,
    "very_high": 400000,
...
def get_latest_station_reading(db: Session, station: str):
    # Backward-compatible helper, but prefer trusted real readings only.
    return get_latest_real_station_reading(db, station)
...
def predict_from_station_reading(db: Session, reading: StationReadingDB, use_live_weather: bool = False):
    # Default behaviour is FAST and stable:
    # use weather values already saved in station_readings by run_scraper_and_upload.py.
    # Only /predict/station uses use_live_weather=True to refresh weather once for a single station.
    if use_live_weather:
        reading = attach_live_weather_to_reading(reading)
...
# ─────────────────────────────────────────────
# MANUAL / EXTERNAL SCRAPER INGEST ENDPOINT
# Used when Railway cannot scrape PMD directly.
# Local curl_cffi scraper uploads readings to Railway backend.
# ─────────────────────────────────────────────

class ManualStationReadingRequest(BaseModel):
    station: str
    discharge: float
    rainfall_mm: Optional[float] = 0.0
    temperature_c: Optional[float] = 25.0
    soil_moisture_mm: Optional[float] = 25.0
    source: Optional[str] = "local_scheduled_scraper"
    reading_time: Optional[str] = None


@app.post("/station-readings/manual")
def create_manual_station_reading(
    payload: ManualStationReadingRequest,
    db: Session = Depends(get_db)
):
    """
    Receives actual live readings from the local scheduled curl_cffi scraper.

    Use this for hosted Railway deployment:
    local scraper -> POST here -> Railway saves to Neon/PostgreSQL.
    """
    try:
        station = normalize_station_name(payload.station)

        parsed_time = None
        if payload.reading_time:
            try:
                parsed_time = datetime.fromisoformat(
                    str(payload.reading_time).replace("Z", "+00:00")
                ).replace(tzinfo=None)
            except Exception:
        ...
        reading = save_station_reading(
            db=db,
            station=station,
            discharge=float(payload.discharge or 0.0),
            rainfall_mm=float(payload.rainfall_mm or 0.0),
            temperature_c=float(payload.temperature_c or 25.0),
            soil_moisture_mm=float(payload.soil_moisture_mm or 25.0),
            reading_time=parsed_time,
            source=payload.source or "local_scheduled_scraper",
        )

        return {
            "success": True,
            "message": "Station reading saved successfully.",
            "reading": serialize_station_reading(reading),
        }

    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to save station reading: {str(e)}"
        )
...
# Lightweight cache for live weather calls.
# This prevents repeated Open-Meteo calls from slowing Railway and exhausting DB connections.
WEATHER_CACHE = {}
WEATHER_CACHE_SECONDS = 600  # 10 minutes


def _reading_has_real_weather(reading: StationReadingDB):
    """
    Returns True when the DB row already contains useful scraper-uploaded weather.
    This avoids unnecessary live API calls.
    """
    try:
        temp = float(reading.temperature_c or 25.0)
        soil = float(reading.soil_moisture_mm or 25.0)
        rain = float(reading.rainfall_mm or 0.0)
    except Exception:
        return False

    # Old bad rows were usually exactly temp=25 and soil=25 with rain=0.
    if temp == 25.0 and soil == 25.0 and rain == 0.0:
        return False

    return True


def _cached_weather_get(station: str):
...
def _cached_weather_set(station: str, data: dict):
    WEATHER_CACHE[station] = {
...
def get_live_station_weather_values(station: str):
    """
    Returns weather values in ML feature names.

    Fast path:
    - use 10-minute in-memory cache

    Live path:
    - call weather.py once for the requested station only

    Fallback:
    - station mock weather, not all 25s
    """
    station = normalize_station_name(station)

    cached = _cached_weather_get(station)
    if cached:
        return cached

    fallback = STATION_WEATHER.get(
        station,
        {"temperature": 25.0, "rainfall": 0.0, "humidity": 50.0, "soil_moisture": 25.0}
    )

    data = None
    try:
        from weather import get_weather_for_station
        data = get_weather_for_station(station)
    except Exception as e:
        print(f"⚠️ weather.py unavailable for prediction weather ({station}): {e}")
    ...
    rainfall = (
        ...
        or data.get("precip_mm")
        or fallback.get("rainfall")
        or 0.0
    )
    soil_moisture = (
        data.get("soil_moisture_mm")
        or data.get("soil_moisture")
        or data.get("soilMoisture")
        or data.get("soil_moisture_percent")
        or fallback.get("soil_moisture")
        or 25.0
    )

    result = {
        "rainfall_mm": _number_or_default(rainfall, 0.0),
        "temperature_c": _number_or_default(temperature, 25.0),
        "soil_moisture_mm": _number_or_default(soil_moisture, 25.0),
    }

    _cached_weather_set(station, result)
    return result

def attach_live_weather_to_reading(reading: StationReadingDB):
    """
    Creates an in-memory reading for direct ML prediction.

    If the latest DB row already has real scraper-uploaded weather, use it directly.
    If it still has old default values (25/25/0), refresh weather from weather.py.
    This fixes /predict/station showing hardcoded weather while avoiding slow
    repeated weather calls in analytics/map endpoints.
    """
    if _reading_has_real_weather(reading):
        return reading

    weather = get_live_station_weather_values(reading.station)
    return StationReadingDB(
        id=reading.id,
        station=reading.station,
        reading_time=reading.reading_time or datetime.utcnow(),
        discharge=float(reading.discharge or 0.0),
        rainfall_mm=float(weather["rainfall_mm"]),
        temperature_c=float(weather["temperature_c"]),
        soil_moisture_mm=float(weather["soil_moisture_mm"]),
        source=reading.source,
        created_at=reading.created_at or datetime.utcnow(),
    )

# ─────────────────────────────────────────────
# HELPERS
# ─────────────────────────────────────────────
def calculate_discharge_risk_percentage(discharge):
    """
    Converts discharge into risk percentage using ML-partner approved thresholds.

    Mapping:
    0–100,000      -> 0–20% Normal
    100,000–150,000 -> 21–40% Low
    150,000–200,000 -> 41–60% Medium
    200,000–400,000 -> 61–80% High
    400,000–600,000 -> 81–95% Very High
    600,000+        -> 96–100% Exceptionally High
    """
    discharge = max(0.0, float(discharge or 0.0))

    if discharge < DISCHARGE_THRESHOLDS["low"]:
        ratio = discharge / DISCHARGE_THRESHOLDS["low"]
        base_risk = ratio * 20.0

    elif discharge < DISCHARGE_THRESHOLDS["medium"]:
        ratio = (
            (discharge - DISCHARGE_THRESHOLDS["low"]) /
            (DISCHARGE_THRESHOLDS["medium"] - DISCHARGE_THRESHOLDS["low"])
        )
        base_risk = 21.0 + ratio * 19.0

    elif discharge < DISCHARGE_THRESHOLDS["high"]:
        ratio = (
            (discharge - DISCHARGE_THRESHOLDS["medium"]) /
            (DISCHARGE_THRESHOLDS["high"] - DISCHARGE_THRESHOLDS["medium"])
        )
        base_risk = 41.0 + ratio * 19.0

    elif discharge < DISCHARGE_THRESHOLDS["very_high"]:
        ratio = (
            (discharge - DISCHARGE_THRESHOLDS["high"]) /
            (DISCHARGE_THRESHOLDS["very_high"] - DISCHARGE_THRESHOLDS["high"])
        )
        base_risk = 61.0 + ratio * 19.0

    elif discharge < DISCHARGE_THRESHOLDS["exceptional"]:
        ratio = (
            (discharge - DISCHARGE_THRESHOLDS["very_high"]) /
            (DISCHARGE_THRESHOLDS["exceptional"] - DISCHARGE_THRESHOLDS["very_high"])
        )
        base_risk = 81.0 + ratio * 14.0

    else:
        base_risk = 100.0

    return round(max(0.0, min(100.0, base_risk)), 2)


def classify_risk_from_percentage(risk_percentage):
...
def discharge_to_percentage(discharge, distance_km=1.0, elevation_m=200):
    """
    Backward-compatible wrapper used across existing endpoints.
    Risk is now based on ML discharge thresholds only.
    distance_km/elevation_m are intentionally ignored to avoid conflict
    between UC distance adjustment and ML discharge risk scale.
    """
    return calculate_discharge_risk_percentage(discharge)
...
def risk_label(pct):
    return classify_risk_from_percentage(pct)
...
# ─────────────────────────────────────────────
# FAST UC / BASIN RISK HELPERS
# ─────────────────────────────────────────────
def get_latest_real_readings_by_station(db: Session):
    """
    Fetches latest real station readings in one small DB pass.

    Old /api/map-risk and /api/basin-overview opened DB sessions repeatedly
    while looping over hundreds of UCs. That exhausted Railway/PostgreSQL
    connections. This helper keeps the same risk logic but reuses 5 station rows.
    """
    readings = {}
    stations = ["Marala", "Khanki", "Qadirabad", "Trimmu", "Panjnad"]
    ...
    for station in stations:
        ...
        if reading:
            readings[station] = reading

    return readings


def build_uc_risk_snapshot_from_readings(
    uc_id,
    uc,
    readings_by_station: Optional[Dict[str, StationReadingDB]] = None,
    predicted_by_station: Optional[Dict[str, float]] = None,
):
    """
    Builds one UC risk snapshot without opening a new DB connection.

    Risk logic is unchanged:
    - current basin/map risk uses ACTUAL current discharge
    - percentage/label/color still use discharge_to_percentage(), risk_label(), risk_color()

    predicted_by_station is optional and only used when caller already computed
    station ML predictions once. This avoids running ML hundreds of times.
    """
    station = uc["station"]
    readings_by_station = readings_by_station or {}
    predicted_by_station = predicted_by_station or {}

    reading = readings_by_station.get(station)

    if reading:
    pass  # fixed indentation
    if reading and reading.discharge is not None:
    pass  # fixed indentation
    if reading and reading.discharge is not None:
        current_discharge = float(reading.discharge)
        risk_percentage = discharge_to_percentage(current_discharge, uc['distance_km'], uc['elevation_m'])
    else:
        # No reading -> assign default Normal risk
        current_discharge = STATION_DEFAULT_DISCHARGE.get(station, 5000)
        risk_percentage = 12  # Force Normal for all monitored UCs
    else:
    if reading and reading.discharge is not None:
        current_discharge = float(reading.discharge)
        risk_percentage = discharge_to_percentage(current_discharge, uc['distance_km'], uc['elevation_m'])
        ...
        current_discharge = STATION_DEFAULT_DISCHARGE.get(station, 5000)
        risk_percentage = 12  # Force Normal for all monitored UCs
        # Assign safe Normal percentage if no reading
        risk_percentage = 12  # Force Normal for all monitored UCs
    else:
    if reading and reading.discharge is not None:
    pass  # fixed indentation
    ...
    predicted_discharge = float(predicted_by_station.get(station, current_discharge))
...
def calculate_uc_risk_snapshot(uc_id, uc):
    ...
    It now closes its DB connection safely and delegates to the fast helper.
    New bulk endpoints should call build_uc_risk_snapshot_from_readings() directly.
    """
    db = SessionLocal()
    try:
        readings_by_station = get_latest_real_readings_by_station(db)
        return build_uc_risk_snapshot_from_readings(uc_id, uc, readings_by_station)
    finally:
        db.close()


def calculate_chenab_overall_risk(db: Optional[Session] = None):
    """
    Calculates one overall risk value for the whole Chenab monitoring area.

    Optimized version:
    - fetches latest station readings once
    - does not open one DB session per UC
    - does not run ML/weather calls per UC
    """
    close_db = False
    if db is None:
        db = SessionLocal()
    ...
    try:
        ...
        snapshots = [
            build_uc_risk_snapshot_from_readings(uc_id, uc, readings_by_station)
            for uc_id, uc in UC_DATABASE.items()
        ]
    finally:
        if close_db:
...
# CHATBOT SPEED CACHE
# ─────────────────────────────────────────────
# Chatbot was slow because every message recalculated basin risk across all UCs.
# This cache keeps the same logic/output, but reuses the context briefly.
CHATBOT_CONTEXT_CACHE = {
    "time": None,
    "data": None,
}
CHATBOT_CONTEXT_CACHE_SECONDS = 60


def get_chatbot_context(db: Session):
    """
    Builds live C-Guard context for the AI chatbot using current backend data.
    Cached briefly so every chatbot message does not recalculate all UC risks.
    """
    now = datetime.utcnow()
    cached_time = CHATBOT_CONTEXT_CACHE.get("time")
    cached_data = CHATBOT_CONTEXT_CACHE.get("data")

    if cached_time and cached_data:
        cache_age = (now - cached_time).total_seconds()
        if cache_age < CHATBOT_CONTEXT_CACHE_SECONDS:
            return cached_data
    ...
    CHATBOT_CONTEXT_CACHE["time"] = now
    CHATBOT_CONTEXT_CACHE["data"] = context_data
    return context_data
...
def basin_overview(db: Session = Depends(get_db)):
    """
    Basin overview endpoint for FloodMap.jsx right-side summary card.
    Optimized to fetch station readings once instead of opening 789 DB sessions.
    """
    summary = {
        "normal": 0,
        "low": 0,
    ...
    for uc_id, uc in UC_DATABASE.items():
        snapshot = build_uc_risk_snapshot_from_readings(uc_id, uc, readings_by_station)
...
def map_risk(db: Session = Depends(get_db)):
    """
    Public API for map hover/tooltips.
    Returns UC risk percentage and level for every UC.
    Optimized: one DB pass for the 5 latest station readings.
    """
    readings_by_station = get_latest_real_readings_by_station(db)
...
def get_forecast(db: Session = Depends(get_db)):
    ...
    Optimized:
    - fetch latest station readings once
    - run ML once per station, not once per UC
    """
    results = []
    readings_by_station = get_latest_real_readings_by_station(db)
    ...
    for station in ["Marala", "Khanki", "Qadirabad", "Trimmu", "Panjnad"]:
        ...
        if reading and MODEL_LOADED:
            try:
                ...
                print(f"⚠️ Forecast failed for {station}: {e}")

        input_discharge = STATION_DEFAULT_DISCHARGE.get(station, 5000)
        station_forecasts[station] = {
            "input_discharge": input_discharge,
            "pred_24h": input_discharge,
            "pred_48h": input_discharge,
            "pred_72h": input_discharge,
            "ml_driven": False,
        }
    ...
    for uc_id, uc in UC_DATABASE.items():
        station = uc["station"]
        station_result = station_forecasts.get(station) or {
            "input_discharge": STATION_DEFAULT_DISCHARGE.get(station, 5000),
            "pred_24h": STATION_DEFAULT_DISCHARGE.get(station, 5000),
            "pred_48h": STATION_DEFAULT_DISCHARGE.get(station, 5000),
            "pred_72h": STATION_DEFAULT_DISCHARGE.get(station, 5000),
            "ml_driven": False,
        }

        pred_24h = station_result["pred_24h"]
        pred_48h = station_result["pred_48h"]
        pred_72h = station_result["pred_72h"]
        input_discharge = station_result["input_discharge"]
        ...
        results.append({
            ...
            "ml_driven": station_result["ml_driven"],
            "timestamp": now_iso,
        })
...
@app.get("/all-ucs")
def get_all_ucs(db: Session = Depends(get_db)):
...
def scrape_now(db: Session = Depends(get_db)):
    ...
    IMPORTANT:
    This endpoint now uses only the real scraper.py flow.
    It does NOT use the old direct PMD fallback because that fallback can
    misread Railway HTML and save wrong station values.

    For production, recommended flow is:
    local curl_cffi scraper -> POST /station-readings/manual -> Railway DB.
    """
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail={
                "message": "scraper.py failed. No fallback was used to avoid saving wrong values.",
                "error": str(scraper_error),
                "recommended_fix": "Run curl_cffi scraper locally and upload readings using /station-readings/manual.",
            },
        )
    ...
    if not rows_to_process:
        raise HTTPException(
            ...
            detail={
                ...
                "note": "No fallback was used to avoid saving wrong values."
            }
        )
    ...
    for row in rows_to_process:
        try:
            ...
            reading = save_station_reading(
                ...
                source=row.get("source") or "curl_cffi_scraper"
            )
    ...
    return make_json_safe({
        ...
        "data_source": "curl_cffi_scraper_or_uploaded_scraper",
...
def predict_station(station_name: str, db: Session = Depends(get_db)):
    ...
    if not reading:
        raise HTTPException(
            status_code=404,
            detail=f"No trusted station reading found for {station_name}. Wait for GitHub Action/local uploader or POST /station-readings/manual."
        )

    return make_json_safe({
        "success": True,
        # Single-station ML prediction refreshes weather from weather.py.
        # Bulk analytics/map endpoints keep using DB-saved weather for speed.
        **predict_from_station_reading(db, reading, use_live_weather=True)
    })
